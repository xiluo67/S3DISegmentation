...
def do_perspective_projection(points_3d, label, target_type, fov_number, save_image, save_label, heights, widths,
    ...
    if (target_type == "left"):
        ...
        camera_position = np.array(
            [np.min(temp[:, 1]) + X / 8 + 0.75* X * widths * 1 / (val + 1), np.min(temp[:, 2]) + Y / 8 + 0.75*Y * heights / 4,
             np.min(temp[:, 0]) + Z / 8 + 0.75* Z * longitudes / 8])
        ...
        camera_position = np.array(
            [np.min(temp[:, 1]) +  X / 8 + 0.75*X * widths / (val + 1), np.min(temp[:, 2]) +  Y / 8 + 0.75*Y * heights / 4,
             np.min(temp[:, 0]) +  Z / 8 + 0.75*Z * longitudes / 8])
        target = camera_position + (0.1, 0, 0)
        reverse = True
...
def gen_the_pp_image(scan, label, scan_path):
    ...
    for f in np.arange(5):
        # fov = 0.2*f + np.pi / param_dict[k][0] - 0.4
        fov = 0.2*f + 1.8
