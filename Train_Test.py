...
if train:
    ...
    # model = UNet(num_classes=num_classes).to(device)
    model.load_state_dict(torch.load('./log/model_20241208_192041_.pth'))
    model.eval()
    ...
    for batch_index, batch in enumerate(test_dataloader):
        ...
        for cls in range(1, num_classes):
            ...
            tp += torch.sum(pred_cls * mask_cls)
            # ioup += intersection / (torch.sum(pred_cls) + torch.sum(mask_cls) - intersection + 1e-8)
